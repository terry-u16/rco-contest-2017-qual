...
impl Input {
            ...
            for col in 0..Self::N {
                ...
                for (i, &adj) in ADJACENTS.iter().enumerate() {
                    ...
                    if next.in_map(Self::N) {
                        graph[next][i] = Some(CoordIndex::new(next.to_index(Self::N)));
                    }
                }
            }
        }
    ...
    fn calc_score(&self, pieces: &[Piece]) -> u32 {
        let mut score = 0;
...
fn main() {
    ...
    for row in 0..Input::N {
        for col in 0..Input::N {
            ...
            piece[i % Input::K] = CoordIndex::new(Coord::new(row, col).to_index(Input::N));
            i += 1;
    ...
    eprintln!("Score = {}", score.calc_score(&output));
}
